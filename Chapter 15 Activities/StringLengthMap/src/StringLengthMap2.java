...
public class StringLengthMap2
{
    public static void main(String[] args)
    {
        ...
        try (Scanner in = new Scanner(new File(filename)))
        {
            ...
            Map<Integer, String> wok = new TreeMap<>();

            while (in.hasNext())
            {
                String word = clean(in.next());
                Integer len = word.length();
                wok.merge(len,word,(oldValue, newValue) -> oldValue.concat(", "+ word));
            ...
            System.out.println(wok);
                        //for (Integer len: wok.keyset())
           // {
            //    System.out.println(len +": "+wok.get(len));
            //}
